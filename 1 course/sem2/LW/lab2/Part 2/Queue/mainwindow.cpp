...
void MainWindow::on_addBtn_clicked()
{
    ...
    if (ui->comboBox->currentIndex() == 0){
        ...
        strings->addItem(text.toStdString());
    }
    ui->lineEdit->clear();
    updateList();
}
...
void MainWindow::updateList(){
    ui->listWidget->clear();
    if (indexCB == 0){
        Node<int> *first = numbers->getFirst();
        Node<int> *current = first;
        while (current != nullptr){
            ui->listWidget->addItem(QString::number(current->item));
            current = current->next;
        }
    }
    else{
        Node<std::string> *first = strings->getFirst();
        Node<std::string> *current = first;
        while (current != nullptr){
            ui->listWidget->addItem(QString::fromStdString(current->item));
            current = current->next;
        }
    }
